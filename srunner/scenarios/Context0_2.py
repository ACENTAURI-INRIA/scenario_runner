...
class Context0_2(BasicScenario):

    """
    This class represent the Context0
    The ego vehicle is passing through an X intersection,
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False,
                 debug_mode=False, criteria_enable=True, timeout=1200):
        """
        ...
        super(Context0_2, self).__init__("Context0_2", ego_vehicles, config, world, debug_mode, criteria_enable=criteria_enable)
    ...
    def _calculate_base_transform(self, _start_distance, waypoint):
        ...
        offset = {"orientation": 290, "position": 50, "z": 0, "k": 1.0}
    ...
    def _initialize_actors(self, config):
        ...
        while True:  # We keep trying to spawn avoiding props
            try:
                ...
                self.transform2 = carla.Transform(carla.Location(250, -243, self._trigger_location.z), carla.Rotation(yaw=250))
                second_vehicle = CarlaDataProvider.request_new_actor('walker.*', self.transform2)
                break
        ...
        #disp_transform2 = carla.Transform(carla.Location(self.transform2.location.x, self.transform2.location.y, self.transform2.location.z - 500), self.transform2.rotation)
        #second_vehicle.set_transform(disp_transform2)
    ...
    def _create_behavior(self):
        ...
        actor_velocity2 = KeepVelocity(self.other_actors[1], self._other_actor_target_velocity, name="walker2 velocity")
        
        actor_drive = DriveDistance(self.other_actors[0], 0.5 * lane_width, name="walker drive distance")
        actor_drive2 = DriveDistance(self.other_actors[1], 0.5 * lane_width, name="walker2 drive distance")
        ...
        actor_start_cross_lane2 = AccelerateToVelocity(self.other_actors[1], 1.0, self._other_actor_target_velocity, name="walker2 crossing lane accelerate velocity")
        
        actor_cross_lane = DriveDistance(self.other_actors[0], lane_width, name="walker drive distance for lane crossing ")
        actor_cross_lane2 = DriveDistance(self.other_actors[1], lane_width, name="walker2 drive distance for lane crossing ")
        
        actor_stop_crossed_lane = StopVehicle(self.other_actors[0], self._other_actor_max_brake, name="walker stop")
        actor_stop_crossed_lane2 = StopVehicle(self.other_actors[1], self._other_actor_max_brake, name="walker2 stop")
        ...
        actor_remove2 = ActorDestroy(self.other_actors[1], name="Destroying walker2")
        ...
        scenario_sequence.add_child(ActorTransformSetter(self.other_actors[1], self.transform2, name='TransformSetterTS3walker'))
        ...
        keep_velocity_other.add_child(actor_start_cross_lane2)
        keep_velocity_other.add_child(actor_cross_lane)
        keep_velocity_other.add_child(actor_cross_lane2)
